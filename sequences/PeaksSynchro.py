...
def compute_sequences(clpeaks, timepeaks, lines, nexp):
    """
    ...
    for line, _, _ in lines:
        ...
        for i in range(peakini, peakend):
            peakstr.append([clpeaks[line][i][0]-1,exp[i-peakini][0]])
...
def generate_synchs(lines, exps, window=15):
    ...
    for nexp, exp in exps:
        sequ = compute_sequences(clpeaks, timepeaks, lines, nexp)
        syn = compute_synchs(sequ, window=window)
        lsynch.append([exp, syn])

    return lsynch

def histograms(lsynch):
    """
    Histograms of the frequencies of the peaks
    :param lsynch:
    :return:
    """
