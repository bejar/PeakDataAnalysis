...
def timeline_overlapped(clpeaks, timepeaks, nfiles, line, gap=300, step=50*300, length=300*300, laplace=0.0, dist='Frobenius'):
    ldiffs = []
    for exp, nfile in nfiles:
        ldiffs.append((compute_timeline_overlapped(exp, clpeaks, timepeaks, line+'-'+nfile, gap=gap, step=step, length=length, laplace=laplace, dist=dist), nfile+'-'+dist))
...
def compute_timeline_overlapped(nexp, clpeaks, timepeaks, nfile, gap=0, step=100, length=1000, laplace=0.0, dist='Frobenius'):
    """
    Computes the time line of the probability matrix of transitions
...
def do_the_job(gap,step, length, laplace, dist):
    for line, clust in aline:
        ...
        timeline_overlapped(clpeaks, timepeaks, nfiles, line, gap=gap, step=step, length=length, laplace=laplace, dist=dist)
