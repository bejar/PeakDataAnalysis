...
if __name__ == '__main__':
    ...
    for expname in lexperiments:
        ...
        for rsensor in ['L5ri', 'L5ci', 'L6ri', 'L6ci']:
            ...
            for dfile in [datainfo.datafiles[9]]:
                print dfile
                ...
                for cl in range(n_clusters):
                    ...
                    t = arange(0.0, peakw, 1)

                    vmax = np.max(cent, axis=0)
                    vmin = np.min(cent, axis=0)
                    vmean = np.mean(cent, axis=0)
                    vstd = np.std(cent, axis=0)
